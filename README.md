...
- Every frame that contains a target object (or a non-target but
  ...
  non-annotated space on the frame as a ‘negative’ so by not annotating
  a target object you will train the model to not recognise them.
...
- The target and non-target class labels need to be absolutely
  consistent across all of your training images (case sensitive). Do not
  edit the ‘labels.txt’ file.

4.  Label empty frames.

- Draw a random box on the frame (its location/size is not important)
  and label it ‘empty’. This is to ensure that during later steps empty
  images are exported alongside other annotated images for training.
  Including empty images if they occur in your dataset is critical so
  that the model learns that empty images can occur.
